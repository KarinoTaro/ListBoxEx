...
namespace dive
{
    ...
    // チェックボックス
    class ListBoxExRowCheckBox : ListBoxExRow
    {
        ...
        public ListBoxExRowCheckBox()
            : this("", "", "", false)
        {
        }

        public ListBoxExRowCheckBox(string name)
            : this(name, "", "", false)
        {
        }

        public ListBoxExRowCheckBox(string name, string text)
            : this(name, text, "", false)
        {
        }

        public ListBoxExRowCheckBox(string name, string text, string description)
            : this(name, text, description, false)
        {
        }

        public ListBoxExRowCheckBox(string name, string text, bool value)
            : this(name, text, "", value)
        {
        }

        public ListBoxExRowCheckBox(string name, string text, string description, bool value)
        {
            _name = name;
            _text = text;
            _desc = description;
        ...
        public override void Draw(Graphics g, int x, int y, bool tinydraw, bool selected)
        {
            ...
            // チェックイメージ描画
            int boxSize = (int)(_imageSize * 0.5);
            g.DrawRectangle(new Pen(Color.Black), x + (_imageSize - boxSize) / 2, y + (_height - boxSize) / 2, boxSize, boxSize);
            if (_value)
            {
                // チェック
                Point[] points = new Point[3];
                points[0] = new Point(x + (_imageSize - boxSize) / 2, y + _height / 2);
                ...
                //g.DrawLines(new Pen(Color.Gray, 11), points);
                //points[0].X = points[0].X + 2;
                //points[0].Y = points[0].Y + 2;
                //points[2].X = points[2].X - 2;
                //points[2].Y = points[2].Y + 2;

                g.DrawLines(new Pen(Color.LightGreen, 8), points);
            }
